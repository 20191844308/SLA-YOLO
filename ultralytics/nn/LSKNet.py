...
__all__ = 'lsknet_t', 'lsknet_s', 'lsknet_m', 'lsknet_l', 'lsknet_x'
...
class OverlapPatchEmbed(nn.Module): #将图像转换为补丁的嵌入形式
    """ Image to Patch Embedding
    """
 
    def __init__(self, img_size=224, patch_size=7, stride=4, in_chans=3, embed_dim=768, norm_cfg=None):
        super().__init__()
        patch_size = to_2tuple(patch_size) #将patch_size转换为元组
...
class LSKNet(nn.Module):
    ...
    def forward(self, x):
        B = x.shape[0]
        outs = []
        for i in range(self.num_stages): #遍历每个阶段
            #从模型中获取当前阶段的图像块嵌入、块和规范化层
...
def lsknet_s(weights=''):
    ...
    return model
#后面的变体是自己编的
def lsknet_m(weights=''):
    model = LSKNet(embed_dims=[128, 256, 512, 1024], depths=[1, 1, 3, 2], drop_rate=0.1, drop_path_rate=0.1)
    if weights:
        model.load_state_dict(update_weight(model.state_dict(), torch.load(weights)['state_dict']))
    return model

def lsknet_l(weights=''):
    model = LSKNet(embed_dims=[128, 256, 512, 1024], depths=[2, 2, 4, 2], drop_rate=0.1, drop_path_rate=0.1)
    if weights:
        model.load_state_dict(update_weight(model.state_dict(), torch.load(weights)['state_dict']))
    return model

def lsknet_x(weights=''):
    model = LSKNet(embed_dims=[128, 256, 512, 1024], depths=[4, 4, 8, 4], drop_rate=0.1, drop_path_rate=0.1)
    if weights:
        model.load_state_dict(update_weight(model.state_dict(), torch.load(weights)['state_dict']))
    return model
