...
class Albumentations:
    ...
    def __init__(self, p=1.0):
        ...
        try:
            ...
            T = [
                A.Blur(p=0.00), #模糊
                A.MedianBlur(p=0.00), #中值模糊
                A.ToGray(p=0.00), #灰度化
                A.CLAHE(p=0.00), #对比度受限自适应直方图均衡化
                #A.Affine(P=0.0), #仿射变换
                A.ElasticTransform(p=0.0), #弹性变换
                A.Rotate(p=0.0), #将图像旋转一个角度
                A.RandomBrightnessContrast(p=0.0), #随机改变图像的亮度和对比度
                A.RandomGamma(p=0.0), #随机Gamma校正，可模拟各种照明条件
                A.ImageCompression(quality_lower=75, p=0.0)]  # transforms 通过图像的 Jpeg、WebP 压缩来降低图像质量
